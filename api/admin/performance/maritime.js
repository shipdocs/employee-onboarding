 ...
 * Provides maritime environment performance analytics
 */
...
async function handler(req, res) {
  ...
  try {
    ...
    // Get maritime-specific metrics
    const maritimeMetrics = await getMaritimeMetrics(startTime, { location, vessel });
    ...
    res.json({
      metrics: {
        performanceScore: calculateMaritimePerformanceScore(maritimeMetrics),
        totalUsers: maritimeMetrics.totalUsers,
        usersAtSea: maritimeMetrics.locationDistribution?.at_sea || 0,
        usersInPort: maritimeMetrics.locationDistribution?.in_port || 0,
        usersOnshore: maritimeMetrics.locationDistribution?.onshore || 0,
        connectionQuality: maritimeMetrics.connectionQuality,
        locationDistribution: maritimeMetrics.locationDistribution,
        averageBandwidth: maritimeMetrics.averageBandwidth,
        averageLatency: maritimeMetrics.averageLatency,
        offlineTime: maritimeMetrics.totalOfflineTime
      },
      trainingPerformance,
      connectionAnalysis,
      devicePatterns,
    ...
    // console.error('Error in maritime performance metrics API:', _error);
    res.status(500).json({
...
/**
 * Get maritime-specific performance metrics
 */
async function getMaritimeMetrics(startTime, filters = {}) {
  try {
    ...
    // console.error('Error getting maritime metrics:', _error);
    return {};
  }
}
...
async function getTrainingPerformanceByLocation(startTime) {
  try {
    const { data: trainingMetrics, error } = await supabase
      .from('maritime_training_metrics')
      .select('*')
...
/**
 * Calculate maritime performance score
 */
