...
class MFAService {
  constructor() {
    ...
    // MFA configuration
    this.config = {
      issuer: process.env.MFA_ISSUER || 'Maritime Onboarding Platform',
  ...
  async useBackupCode(userId, usedEncryptedCode) {
    try {
      const settingsResult = await db.query('SELECT backup_codes FROM user_mfa_settings WHERE user_id = $1', [userId]);
    const settings = settingsResult.rows[0];
    const fetchError = !settings;
