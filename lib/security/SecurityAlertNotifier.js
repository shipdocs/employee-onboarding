...
For urgent issues, contact: ${process.env.SECURITY_EMAIL || 'security@company.com'}
  ...
  setRecipients(type, recipients) {
    this.config.recipients[type] = recipients;
  }
  ...
  getEmailStats() {
    ...
    return {
      ...
      maxEmailsPerHour: this.config.rateLimiting.maxEmailsPerHour,
      remainingEmails: Math.max(0, this.config.rateLimiting.maxEmailsPerHour - recentEmails.length),
