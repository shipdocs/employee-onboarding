...
const { SecurityAlertNotifier } = require('./SecurityAlertNotifier');
...
class SecurityMonitoringService extends EventEmitter {
  constructor() {
    ...
    this.alertNotifier = new SecurityAlertNotifier();
  }
  ...
  async checkAlertThresholds() {
    ...
    if (newAlerts.length > 0) {
      ...
      // Process alerts in parallel with proper async handling
      await Promise.all(newAlerts.map(async (alert) => {
        ...
        try {
          await this.alertNotifier.sendAlertEmail(alert);
        } catch (error) {
