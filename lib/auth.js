/**
 * @file auth.js
 * @brief Core authentication and security library for the Maritime Onboarding System
 *
 * @details This library provides comprehensive authentication and security functionality
 * for the maritime onboarding system. It implements JWT-based authentication, magic link
 * generation, session management, and security features designed for maritime environments
 * where traditional password-based authentication may be challenging.
 *
 * **Core Authentication Features:**
 * - JWT token generation and validation
 * - Magic link token creation for passwordless authentication
 ...
 * - IP address validation and binding
 ...
 * - Extended session durations for long voyages
 * - Device-specific authentication for shared terminals
 * - Emergency access procedures for critical situations
 * - Compliance with maritime security regulations
 *
 * **Integration Points:**
 * - Database integration for user and session storage
 * - Email service integration for magic link delivery
 * - Security logging and audit trail systems
 * - Configuration management for security parameters
 ...
 * @since 2024
 *
 * @see emailService For magic link delivery
 * @see securityLogger For security event logging
 * @see EnhancedSessionManager For session management
 */
...
/**
 * @brief Generate a secure magic link token for passwordless authentication
 * @details Creates a cryptographically secure random token used for magic link authentication.
 * The token is 64 characters long (32 bytes in hex format) and suitable for one-time use.
...
/**
 * @brief Generate JWT token with enhanced security binding and session management
 * @details Creates a secure JWT token with user information, session binding, and device fingerprinting.
 * The token includes security features like IP binding, device fingerprinting, and session management.
 * @param {Object} user - User object containing id, email, role, and other user data
 * @param {Object} req - HTTP request object for security binding (optional)
 * @return {Promise<Object>} Object containing success status, token, and session information
 * @throws {Error} If JWT generation fails or user data is invalid
 * @example
 * const result = await generateJWT(user, req);
 * if (result.success) {
...
async function verifyAuth(req, res) {
  try {
    ...
    if (user.role === 'manager') {
      const managerPermissionsResult = await db.query('SELECT permission_key FROM manager_permissions WHERE manager_id = $1', [user.id]);
    const managerPermissions = managerPermissionsResult.rows;
...
async function rotateRefreshToken(oldRefreshToken, req = null) {
  try {
    ...
    // Get user details
    const userResult = await db.query('SELECT * FROM users WHERE id = $1', [validation.userId]);
    const user = userResult.rows[0];
    const userError = !user;
