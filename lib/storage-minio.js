...
if (!process.env.MINIO_ENDPOINT || !process.env.MINIO_PORT || 
  ...
  throw new Error('Missing required MinIO environment variables. Please set MINIO_ENDPOINT, MINIO_PORT, MINIO_ACCESS_KEY, and MINIO_SECRET_KEY');
}

// Validate MinIO credentials are not defaults
const defaultCredentials = ['minioadmin', 'admin', 'password'];
if (defaultCredentials.includes(process.env.MINIO_ACCESS_KEY.toLowerCase()) ||
    defaultCredentials.includes(process.env.MINIO_SECRET_KEY.toLowerCase())) {
  console.error('❌ SECURITY WARNING: MinIO is using default credentials. Please change them immediately!');
  if (process.env.NODE_ENV === 'production') {
    throw new Error('Cannot use default MinIO credentials in production');
  }
}

// Initialize MinIO client with secure configuration
