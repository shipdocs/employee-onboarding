...
class AccessReportService {
  ...
  async generateUserAccessReport(targetUserId, startDate, endDate, requestedBy) {
    try {
      ...
      const userDataResult = await db.query('SELECT id, email, first_name, last_name, role, created_at FROM users WHERE id = $1', [targetUserId]);
  ...
  async generateCompanyAccessReport(companyId, startDate, endDate, requestedBy) {
    try {
      ...
      const companyUsersResult = await db.query('SELECT id, email, first_name, last_name, role FROM users WHERE company_id = $1', [companyId]);
    const companyUsers = companyUsersResult.rows;
