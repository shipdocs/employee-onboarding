...
const externalLoggingService = require('../services/externalLoggingService');
...
/**
 * Log security event to database and external logging
 */
async function logSecurityEvent(eventData) {
  try {
    ...
    // This is non-blocking and won't affect the request
    externalLoggingService.logSecurityEvent(eventData).catch(err => {
      console.error('External logging failed:', err.message);
    });
  } catch (err) {
