...
describe('Security Improvements', () => {
  describe('Attack Pattern Detection', () => {
    ...
    test('should not flag legitimate inputs', () => {
      const legitimateInputs = [
        "user@example.com",
        "Hello world",
        "SELECT * FROM products WHERE category = 'electronics'", // This might trigger, but it's in quotes
        "My password is secure123!",
        "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
      ];

      // Note: Some legitimate inputs might trigger false positives, 
      // which is why we need careful tuning of patterns
    });
  });
  ...
  describe('Security Event Logging', () => {
    test('should generate unique event IDs', () => {
      const { generateEventId } = require('../../../lib/middleware/securityMonitoring');
