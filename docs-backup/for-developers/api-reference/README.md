...
```bash
curl -X POST https://onboarding.burando.online/api/auth/magic-link \
...
2. **Verify Magic Link**
```bash
curl -X POST https://onboarding.burando.online/api/auth/verify-magic-link \
  -H "Content-Type: application/json" \
...
```bash
curl -X GET https://onboarding.burando.online/api/crew/550e8400/dashboard \
...
```bash
curl -X POST https://onboarding.burando.online/api/crew/550e8400/documents/upload \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIs..." \
...
curl -X GET https://onboarding.burando.online/api/admin/dashboard \
...
```bash
curl -X GET "https://onboarding.burando.online/api/manager/crew?page=2&limit=10&status=active" \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIs..."
```
...
# Usage
api = OnboardingAPI('https://onboarding.burando.online', token)
