...
#### Admin Login
```bash
curl -X POST https://api.onboarding.burando.online/auth/admin-login \
...
#### Magic Link Login
```bash
curl -X POST https://api.onboarding.burando.online/auth/magic-login \
  -H "Content-Type: application/json" \
  -d '{
    "token": "a1b2c3d4e5f6..."
  }'
...
#### Verify Token
```bash
curl -X GET https://api.onboarding.burando.online/auth/verify \
